#!/usr/bin/env python3
"""
PerformTask_service.py
ROS 2 Service Node for Pick-and-Place using Trigger service.

run using 
ros2 service call /PerformTask std_srvs/srv/Trigger "{}"
"""
...
class PickPlaceService(Node):
    def __init__(self):
        super().__init__("PerformTask_service")
        ...
        self.srv = self.create_service(Trigger, 'PerformTask', self.PerformTask_callback)

        self.get_logger().info("Pick & Place service node ready! Call /PerformTask to execute.")
    ...
    def move_gripper(self, position, effort=2.0, settle_time=2.0):
    ...
    # ---- Service Callback ----
    def PerformTask_callback(self, request, response):
        
        self.get_logger().info("Service called: /PerformTask")
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().info("Shutting down PerformTask_service node.")
    finally:
        node.destroy_node()
        rclpy.shutdown()
